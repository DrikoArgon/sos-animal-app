...
const membersToAddArray = []

export default class FindMemberToAddScreen extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            dataSource: ds.cloneWithRows(membersToAddArray),
            modalVisible: false,
            peopleFound: true,
            loading: true
        }
    ...
    toMemberProfile(memberData) {

        var isMember = false
        var membersArray = FirebaseRequest.getCurrentGroupMembers()
        

        if (membersArray) {
            for (var i = 0; i < membersArray.length; i++) {

                if (membersArray[i].key === memberData.key) {
        ...
        memberData.isGroupLooking = true

        this.props.toRoute(Routes.friendProfile(memberData, isMember,true))
    }
    ...
    componentDidMount(){
        ...
        FirebaseRequest.fetchPeople(true)
            .then((peopleArray) => {
                membersToAddArray = peopleArray
    ...
    _renderRow(rowData){
        return(
                <TouchableOpacity
                    ...
                    onPress={() => this.toMemberProfile(rowData)}
                >
    ...
    searchFriend(){
        ...
        if(this.searchString == ""){
            ...
            for(var i = 0; i < membersToAddArray.length; i++){
                if(membersToAddArray[i].name.indexOf(this.searchString) !== -1){
                    newArray.push(membersToAddArray[i])
                }
            }
