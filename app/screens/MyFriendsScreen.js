...
export default class MyFriendsScreen extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            dataSource: ds.cloneWithRows(myFriends),
            modalVisible: false,
            loading: true,
            friendsFound: true
        }
    ...
    componentDidMount() {
        ...
        if (FirebaseRequest.getCurrentUserFriends()) {

            myFriends = FirebaseRequest.getCurrentUserFriends()
            ...
            FirebaseRequest.listenToNewFriendsAdded(this.updateFriendsList)
        }
        else {
        ...
        FirebaseRequest.listenToUserFriendRemoved(this.updateFriendsListWhenRemoved)
    }

    componentWillUnmount(){
        FirebaseRequest.removeNewFriendsListener()
    }
    ...
    updateFriendsListWhenRemoved = (friendRemoved) => {
        ...
        for(var i = 0; i < myFriends.length; i++){
            if(myFriends[i].key === friendRemoved.key){
                index = i
                break
            }
        }
    ...
    searchFriend(){
        ...
        if(this.searchString == ""){
            ...
            for(var i = 0; i < myFriends.length; i++){
                if(myFriends[i].name.indexOf(this.searchString) !== -1){
                    newArray.push(myFriends[i])
                }
            }
